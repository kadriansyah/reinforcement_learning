...
The algorithm is tested on the FrozenLake-v0 OpenAI gym task
...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau):
        ...
        # Actor Network
        self.inputs, self.out, self.scaled_out = self.create_actor_network('actor_online')
    ...
    def create_actor_network(self, name):
        ...
        # hidden layer
        net = tf.contrib.layers.fully_connected(tf.contrib.layers.fully_connected(inputs, 400), 300)

        # output layer
        w_init = tf.random_uniform(shape=[300, self.a_dim], minval=-0.003, maxval=0.003, dtype=tf.float32)
        W = tf.get_variable(
                "W_"+ name,
                initializer=w_init)
        b = tf.get_variable(
                "b_"+ name,
                initializer=tf.constant(0.1, shape=[self.a_dim]))
        out = tf.tanh(tf.nn.xw_plus_b(net, W, b), name="out_"+ name)
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, num_actor_vars):
        ...
        self.inputs, self.action, self.out = self.create_critic_network('online')
    ...
    def create_critic_network(self, name):
        ...
        # inputs layer_1
        W1_inputs = tf.get_variable(
                        "W1_inputs_"+ name,
                        shape=[self.s_dim, 400],
                        initializer=tf.contrib.layers.xavier_initializer())
        b1_inputs = tf.get_variable(
                        "b1_inputs_"+ name,
                        initializer=tf.constant(0.1, shape=[400]))
        inputs_layer_1 = tf.nn.relu(tf.nn.xw_plus_b(inputs, W1_inputs, b1_inputs), name="inputs_layer_1_"+ name)

        # inputs layer_2
        W2_inputs = tf.get_variable(
                        "W2_inputs_"+ name,
                        shape=[400, 300],
                        initializer=tf.contrib.layers.xavier_initializer())
        inputs_layer_2 = tf.matmul(inputs_layer_1, W2_inputs)

        # action layer_1
        W1_action = tf.get_variable(
                        "W1_action_"+ name,
                        shape=[self.a_dim, 300],
                        initializer=tf.contrib.layers.xavier_initializer())
        b1_action = tf.get_variable(
                        "b1_action_"+ name,
                        initializer=tf.constant(0.1, shape=[300]))
        action_layer_1 = tf.matmul(action, W1_action) + b1_action

        # net activation
        net = tf.nn.relu(inputs_layer_2 + action_layer_1)

        # output layer
        weights_initializer = tf.random_uniform(shape=[300, 1], minval=-0.003, maxval=0.003, dtype=tf.float32)
        W = tf.get_variable(
                "W_"+ name,
        ...
        out = tf.nn.relu(tf.nn.xw_plus_b(net, W, b), name="out_"+ name)
...
def train(sess, env, actor, critic):
    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(MAX_EP_STEPS):
            ...
            replay_buffer.add(
                np.reshape(s, (actor.s_dim,)), np.reshape(a, (actor.a_dim,)), r, done, np.reshape(s2, (actor.s_dim,)))
